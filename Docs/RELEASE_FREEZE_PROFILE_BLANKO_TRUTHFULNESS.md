...
Neu bindet `tests/security/test_auth_profile_none_fail_closed.py` den bereits zentralisierten inneren Auth-Resolver an einen expliziten Fehlzustandsvertrag. Der Test beweist ohne Produktfunktionsänderung, dass `None`, leere, nur aus Leerzeichen bestehende oder nicht-stringförmige Profil-IDs nicht akzeptiert werden; dass Revisionsprüfung ohne Profil die Datenbank nicht berührt; dass ein vorhandener Auth-Cache bei fehlendem Profil verworfen wird; dass ein Request mit formal korrekten Basic-Auth-Daten keine PIN-Verifikation erreicht; und dass die Erststart-Härtung mit `AUTH_PROFILE_ID_UNAVAILABLE` stoppt, bevor Profilzugriff oder PIN-Mutation stattfinden können.

Damit ist die Reihenfolge des späteren Runtime-Umbaus technisch abgesichert: **beide Sicherheitslagen zentralisiert → `profile_id=None` fail-closed beweisen → kontrollierten profilfreien Laufzeitzustand entwerfen → danach realen Server-Fallback entfernen → Runtime-/Browser-Evidence → erst dann sichtbar Blanco**.

## CI-Durchsetzung

Der Vertrag wird zusätzlich durch `.github/workflows/profile-blanco-truthfulness.yml` bei Pull Requests und Pushes auf `main` automatisch ausgeführt. Der Workflow verwendet ausschließlich fest auf Commit-SHAs gepinnte GitHub Actions, persistiert keine Checkout-Zugangsdaten und ruft den Truthfulness-Test, das Auth-Profil-Containment und den neuen Missing-Profile-Fail-Closed-Vertrag ohne externe Testabhängigkeit direkt mit Python auf.

Wichtig: Die Testdateien besitzen eigene kleine Direct-Runner. Ein direkter Aufruf führt die jeweilige Vertragsprüfung tatsächlich aus, gibt PASS/FAIL bzw. eine Zusammenfassung aus und beendet den Prozess bei einem Fehler mit Exit-Code 1. Das Auth-Profil-Containment führt dabei die reale Quelltextprüfung, beide Resolver-Contracts sowie beide Mutationstests aus. Der Missing-Profile-Test führt fünf konkrete Verhaltensprüfungen aus. Damit kann ein grüner Workflow nicht allein dadurch entstehen, dass Testfunktionen nur definiert, aber nie aufgerufen werden, der Detektor eine neue Zugriffsstelle übersieht oder ein fehlendes Profil versehentlich doch Datenbank-, PIN- oder Cache-Vertrauen erhält.
...
Damit sind die Tests nicht nur vorhandene lokale Evidence, sondern tatsächlich ausführende, fail-closed CI-Gates gegen spätere UI-/Backend-/Auth-Drift, gegen eine erneute Ausbreitung der Profilkopplung, gegen einen falsch grünen Containment-Detektor, gegen eine versehentliche Auth-Freigabe ohne Profil und gegen Verhaltensregressionen der zentralisierten Auth-Grenze.
...
Bis dieser Runtime-Vertrag vollständig implementiert und getestet ist, bleibt die Profil-Startänderung **Draft / NO-GO**. Der ausführbare Truthfulness-Test, das Profil/Auth-Kopplungs-Gate, die in beiden Sicherheitslagen zentralisierten fail-closed Profilgrenzen, das mutationserprobte Dependency-Containment, der explizite Missing-Profile-Fail-Closed-Vertrag, die Auth-Verhaltensregressionen, ihre CI-Durchsetzung und die verständliche Ladeanzeige verhindern gefährliche bzw. verwirrende Zwischenzustände; sie ersetzen den Runtime-Blanco-Nachweis nicht.
