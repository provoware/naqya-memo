...
Zusätzlich ist der geschützte Desktop-Start noch an ein konkretes aktives Profil gekoppelt: `app/secure_server.py` verwendet `base.PROFILE_ID` für PIN-Prüfung, Erststart-Härtung und Sicherheitszustand. Der äußere Response-Layer war ebenfalls direkt an `secure.base.PROFILE_ID` gekoppelt; dieser Zugriff ist jetzt in genau einem fail-closed Resolver `_response_auth_profile_id()` gekapselt. Fehlt dort eine eindeutige nichtleere Profil-ID, liefert der Resolver `None` und der isolierte Sicherheitszustand wird nicht abgefragt. Ein Entfernen des Server-Fallbacks ohne vorherige Entkopplung des inneren Auth-Layers könnte weiterhin den Desktop-Zugriff beschädigen oder einen inkonsistenten Sicherheitszustand erzeugen.
...
Zusätzlich prüft `tests/security/test_auth_profile_dependency_containment.py` per Python-AST (Syntaxbaum), dass direkte `PROFILE_ID`-Zugriffe in den Desktop-Sicherheitsservern ausschließlich innerhalb der bekannten Auth-Grenzen liegen. Im äußeren `secure_response_server.py` ist jetzt **genau ein** direkter Zugriff erlaubt und verpflichtend: `secure.base.PROFILE_ID` darf ausschließlich in `_response_auth_profile_id()` vorkommen. Jede erneute direkte Verwendung im eigentlichen Sicherheitszustands-Reader oder an anderer Stelle blockiert fail-closed.

Das Containment begrenzt außerdem die **Anzahl direkter Zugriffe pro erlaubter Auth-Grenze**. Damit reicht es nicht aus, einen neuen `PROFILE_ID`-Zugriff in eine schon freigegebene Funktion zu setzen: Für jede bestehende Kopplung gilt ein Oberlimit. Die Anzahl darf im Zuge der Zentralisierung sinken, aber nicht unbemerkt wachsen. So wird die technische Schuld bis zum geplanten zentralen Auth-Profil-Grenzpunkt sowohl räumlich als auch mengenmäßig eingefroren.

Das Containment-Gate prüft seinen eigenen Detektor mit absichtlich eingeschleusten Mutationen: Ein synthetischer neuer Zugriff `base.PROFILE_ID` in einem nicht erlaubten Auth-Helfer muss eindeutig als Verstoß erkannt werden; zusätzlich muss ein zweiter direkter Zugriff innerhalb der bereits erlaubten Funktion `_profile_revision` am Mengenlimit scheitern. Ergänzend beweist ein eigener Contract, dass der Response-Layer tatsächlich nur noch den einen Resolver als direkten Profilzugriff besitzt.

Damit ist die Reihenfolge des späteren Runtime-Umbaus technisch abgesichert: **Response-Layer zentralisieren → inneren Auth-Vertrag profiloptional und zentral machen → danach realen Server auf Blanco umstellen**.
...
Bis dieser Runtime-Vertrag vollständig implementiert und getestet ist, bleibt die Profil-Startänderung **Draft / NO-GO**. Der ausführbare Truthfulness-Test, das Profil/Auth-Kopplungs-Gate, die bereits zentralisierte Response-Profilgrenze, das räumlich und mengenmäßig begrenzte sowie mutationserprobte Dependency-Containment, ihre CI-Durchsetzung und die verständliche Ladeanzeige verhindern gefährliche bzw. verwirrende Zwischenzustände; sie ersetzen den Runtime-Blanco-Nachweis nicht.
