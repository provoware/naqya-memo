...
import subprocess, sys, os, time, json, urllib.request, urllib.error, tempfile, signal, socket
...
def run():
    ...
    p=subprocess.Popen([sys.executable,'-S',str(SERVER),'--no-browser'],env=env,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    try:
        wait()
