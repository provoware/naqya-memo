...
def replace_pin_hash_without_revision_or_timestamp_change(project: Path, new_pin: str) -> tuple[int, str]:
    db = project / 'daten' / 'core.sqlite3'
    con = sqlite3.connect(db, timeout=3)
    try:
        row = con.execute(
            "SELECT id, revision, created_at, pin_hash FROM profiles WHERE status='ACTIVE' LIMIT 1"
        ).fetchone()
        assert row is not None, 'active profile missing'
        profile_id, revision, created_at, old_hash = row
        new_hash = hash_pin(new_pin)
        assert new_hash != old_hash
        with con:
            con.execute("UPDATE profiles SET pin_hash=? WHERE id=?", (new_hash, profile_id))
        row_after = con.execute(
            "SELECT revision, created_at, pin_hash FROM profiles WHERE id=?", (profile_id,)
        ).fetchone()
        assert row_after is not None
        assert int(row_after[0]) == int(revision), row_after
        assert str(row_after[1]) == str(created_at), row_after
        assert str(row_after[2]) == new_hash, row_after
        return int(revision), str(created_at)
    finally:
        con.close()
...
def main() -> None:
    port = free_port()
    with tempfile.TemporaryDirectory(prefix='provoware-auth-cache-fingerprint-') as td:
        ...
        try:
            ...
            status, body = request(port, first_pin)
            assert status == 200, (status, body[:160])
            print('PASS unchanged credential fingerprint permits the cached credential')

            new_pin = choose_new_pin(first_pin)
            revision, created_at = replace_pin_hash_without_revision_or_timestamp_change(project, new_pin)
            print(
                'PASS fixture changes only pin_hash while preserving '
                f'revision={revision} and created_at={created_at}'
            )

            status, _ = request(port, first_pin)
            assert status == 401, status
            print('PASS predecessor cached PIN is rejected when only credential material changes')

            status, body = request(port, new_pin)
            assert status == 200, (status, body[:160])
            print('PASS replacement PIN authenticates under the new credential fingerprint epoch')
