...
FORBIDDEN_PYTHON_IMPORTS = {
    "aiohttp", "ftplib", "http.client", "httpx", "requests", "smtplib",
    "socket", "telnetlib", "urllib.request", "xmlrpc.client",
}
FORBIDDEN_PYTHON_CALL_PREFIXES = ("socket.", "urllib.request.", "http.client.")
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                if alias.name in FORBIDDEN_PYTHON_IMPORTS:
                    findings.append(f"{path.relative_to(ROOT)}:{node.lineno}: direct network-capable import {alias.name}")
        elif isinstance(node, ast.ImportFrom):
            module = node.module or ""
            if module in FORBIDDEN_PYTHON_IMPORTS:
                findings.append(f"{path.relative_to(ROOT)}:{node.lineno}: direct network-capable import {module}")
        elif isinstance(node, ast.Call):
            name = dotted_name(node.func) or ""
            if name.startswith(FORBIDDEN_PYTHON_CALL_PREFIXES):
                findings.append(f"{path.relative_to(ROOT)}:{node.lineno}: direct network-capable call {name}")
    return findings
...
def check_javascript(path: Path) -> list[str]:
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if FORBIDDEN_JS_IMPORT.search(stripped):
            findings.append(f"{path.relative_to(ROOT)}:{line_number}: direct Node network module import")
        if FORBIDDEN_JS_CALL.search(stripped):
            findings.append(f"{path.relative_to(ROOT)}:{line_number}: direct browser/network client call")
    return findings
