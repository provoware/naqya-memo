...
import os,sys,socket,tempfile,subprocess,time,json,urllib.request,traceback,shutil,importlib.util

ROOT=Path(__file__).resolve().parents[2]
VERSION=json.loads((ROOT/"registry/VERSION.json").read_text(encoding="utf-8"))["version"]
OUT=ROOT/"registry"/"evidence"/"v0.12.2.5"/"browser"
...
browser_path=os.environ.get("PROVOWARE_CHROMIUM") or next(
    (p for p in (
...
if importlib.util.find_spec("playwright") is None or not browser_path:
    result={
        "version":VERSION,
        "status":"BLOCKED",
        "cases":[],
        "reason":"Playwright oder Chromium/Chrome ist auf diesem Rechner nicht verfügbar.",
        "browser_path":browser_path,
        "playwright_available":importlib.util.find_spec("playwright") is not None,
    }
    (OUT/"ALL_VIEWS_VISUAL_ACCEPTANCE.json").write_text(
        json.dumps(result,indent=2,ensure_ascii=False)+"\n",encoding="utf-8"
    )
    print(json.dumps({"status":"BLOCKED","reason":result["reason"]},ensure_ascii=False))
    raise SystemExit(3)
...
raise SystemExit(0 if result["status"]=="PASS" else (3 if result["status"]=="BLOCKED" else 2))
