...
import json, os, sys, threading, webbrowser, subprocess, shutil, traceback, datetime, mimetypes
...
def api_state():
    return {
        'version':'0.12.0-RC-HARDENING','profile':{'id':PROFILE_ID,'name':PROFILE_NAME},
...
class Handler(SimpleHTTPRequestHandler):
    server_version='ProvowareV011/1.0'
    ...
    def do_GET(self):
        ...
        try:
            ...
            if path=='/api/health': return self._ok({'integrity':store.integrity_check(),
        'asset_quota':asset_manager.quota_status(),
...
def run(port=8765,open_browser=True):
    ...
    print(f'OI - PROVOWARE - IO V0.12 läuft: {url}',flush=True)
